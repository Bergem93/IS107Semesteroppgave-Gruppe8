...
echo "<br>";
echo "<br>";
$sidet = $_POST['postinnlegg']; 
$melding = $_POST['Melding2']; 
$insertPos=0;  // variabel for å lagre markør posisjon

$file=fopen("index.html","r+") or exit("...kødda, det her systemet fungerer ikke LITT engang. Beklager. Prøv igjen senere."); // variabel for åpning av fila og skrivemodus
	
while (!feof($file)) { //sålenge markøren ikke er på slutten av fila
	$line=fgets($file);

	if (strpos($line, '<ul class="teste">')!==false) { //strpos leter i line etter første instans av body
...
echo "Innlegget er blitt publisert på forsiden, wohooo!";
